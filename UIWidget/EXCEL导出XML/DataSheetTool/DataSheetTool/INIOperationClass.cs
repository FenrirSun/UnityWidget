...
public class INIOperationClass
{
    ...
    public static string INIGetStringValue(string iniFile, string section, string key, string defaultValue)
    {
        ...
        try{
            bytesReturned = INIOperationClass.GetPrivateProfileString(section, key, defaultValue, sb, SIZE, iniFile);
        }
        catch
        {
            throw new ArgumentException("读取iniFile文件失败", "iniFile");
        }
        if (bytesReturned != 0)
        {
            value = sb.ToString();
